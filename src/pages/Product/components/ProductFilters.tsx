...
interface ProductFiltersProps {
  ...
  onPriceFilter: (index: number) => void;
...
export default function ProductFilters({
    ...
    const fetchCategories = async () => {
      try {
        let response = await axios.get(
          `${VITE_API_BASE_URL}/api/v1/categories`
        );
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (name === 'largeCategory') {
      ...
      onFilterChange({
        ...filters,
        categoryId: value, // "전체" 선택 시 undefined로 설정
        largeCategory: value,
        mediumCategory: '',
        smallCategory: '',
      });
      ...
      onFilterChange({
        ...filters,
        categoryId: value || largeCategory, // "전체" 선택 시 대분류로 fallback
      ...
      onFilterChange({
        ...filters,
        categoryId: value || mediumCategory || largeCategory,
        smallCategory: value,
      });
    }
  };
    ...
    <aside className='w-1/4 p-4 bg-white shadow-lg rounded-lg sticky top-24 self-start'>
      ...
      <div className='mb-4'>
        ...
        <div className='flex flex-col gap-2'>
          {priceRangeOption.map((range, index) => (

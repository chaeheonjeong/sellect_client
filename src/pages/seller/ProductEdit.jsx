...
function ProductEdit() {
    ...
    const fetchData = async () => {
      try {
        const productResponse = await axios.get(
          `${VITE_API_BASE_URL}/api/v1/seller/products/${productId}`,
          {
        ...
        setFormData({
          largeCategoryId: product.category_id
  ...
  const handleImagesChange = (newImages) => {
    ...
    setPreviewImages([
      ...previewImages.filter((img) => img.isExisting),
      ...newPreviewImages,
    ]);
  };
  ...
  const handleRemoveImage = (imageId) => {
    ...
    setFormData((prev) => ({
      ...
      existingImages: prev.existingImages.filter(
        (img) => img.image_url !== removedImage?.url
      ),
      images: prev.images.filter(
        (img) => !removedImage?.file || img !== removedImage.file
      ),
    }));
  ...
  const handleImageSubmit = async (e) => {
    ...
    formDataToSend.append(
      'modify_request',
      new Blob([JSON.stringify(updateImageRequest)], {
    ...
    formData.images.forEach((file) => {
      ...
      const matchingPreview = previewImages.find(
        (img) => !img.isExisting && img.file === file
      );
      const uuid = matchingPreview ? matchingPreview.uuid : uuidv4();
      formDataToSend.append(
        'images',
        new File([file], `${uuid}.${fileExtension}`, { type: file.type })
      );
    });
    ...
    try {
      const response = await axios.put(
        `${VITE_API_BASE_URL}/api/v1/products/images`,
        formDataToSend,
        {
          withCredentials: true,
          headers: {
